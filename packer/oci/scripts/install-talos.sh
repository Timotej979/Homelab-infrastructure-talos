...
show_help() {
    ...
    echo
    echo "Fetch Oracle image from the Talos Factory API."
    echo
    echo "Arguments:"
...
fetch_image_from_talos_factory() {
    ...
    log_info "Fetching the image from the Talos Factory API ..."
    curl -X GET "$(printf "%s/image/%s/%s/oracle-%s.raw.xz" "$TALOS_IMAGE_FACTORY_URL" "$TALOS_SCHEMATIC_ID" "$TALOS_VERSION" "$TALOS_MACHINE_TYPE")" -o talos-img.raw.xz || {
