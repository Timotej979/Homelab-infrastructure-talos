...
show_help() {
    ...
    echo
    echo "Fetch Azure image from the Talos Factory API."
    echo
    echo "Arguments:"
...
fetch_image_from_talos_factory() {
    ...
    log_info "Fetching the image from the Talos Factory API ..."
    curl -X GET $TALOS_IMAGE_FACTORY_URL/image/$TALOS_SCHEMATIC_ID/$TALOS_VERSION/azure-$TALOS_MACHINE_TYPE.raw.xz -o talos-aws-$TALOS_MACHINE_TYPE.raw.xz
    ...
    if [[ $? -ne 0 ]]; then
        ...
        log_success "Image for Azure cloud for $TALOS_MACHINE_TYPE AMI fetched successfully"
    fi
}
