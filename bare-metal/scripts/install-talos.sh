...
show_help() {
    ...
    echo
    echo "Fetch bare-metal image from the Talos Factory API."
    echo
    echo "Arguments:"
    ...
    echo "                                 Options are arm64/amd64 (Default is amd64)."
...
fetch_image_from_talos_factory() {
    ...
    if [[ $? -ne 0 ]]; then
        ...
        log_success "Machine image for bare-metal for $TALOS_MACHINE_TYPE fetched successfully"
    fi
}
