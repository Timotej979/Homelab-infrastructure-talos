...
show_help() {
    ...
    echo
    echo "Fetch Digital Ocean image from the Talos Factory API."
    echo
    echo "Arguments:"
...
fetch_image_from_talos_factory() {
    ...
    log_info "Fetching the image from the Talos Factory API ..."
    curl -X GET $TALOS_IMAGE_FACTORY_URL/image/$TALOS_SCHEMATIC_ID/$TALOS_VERSION/digital-ocean-$TALOS_MACHINE_TYPE.raw.gz -o talos-aws-$TALOS_MACHINE_TYPE.raw.xz
    ...
    if [[ $? -ne 0 ]]; then
        ...
        log_success "Image for Digital Ocean cloud for $TALOS_MACHINE_TYPE AMI fetched successfully"
    fi
}
